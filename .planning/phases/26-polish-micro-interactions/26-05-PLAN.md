...
must_haves:
  ...
  key_links:
    ...
    - from: "admin/layout.tsx"
      ...
      via: "renders CommandPalette + ShortcutsReference"
...
<task type="auto">
  <name>Task 3: Wire command palette and shortcuts into all dashboard layouts</name>
  <files>apps/web/src/app/admin/layout.tsx, apps/web/src/app/va/layout.tsx, apps/web/src/app/client/layout.tsx</files>
  <action>
Integrate CommandPalette, ShortcutsReference, and useGlobalShortcuts into all three dashboard layouts.
...
function AdminLayoutShortcuts({ children }: { children: React.ReactNode }) {
  const { commandOpen, setCommandOpen, shortcutsOpen, setShortcutsOpen } = useGlobalShortcuts({ basePath: "/admin" })

  return (
    <>
      {children}
      <CommandPalette open={commandOpen} onOpenChange={setCommandOpen} basePath="/admin" />
      <ShortcutsReference open={shortcutsOpen} onOpenChange={setShortcutsOpen} />
    </>
  )
}
...
- CommandPalette and ShortcutsReference render as portaled dialogs (no layout impact)
...
- Don't remove or modify any existing components (AppSidebar, BreadcrumbNav, SyncProvider, etc.)
  ...
  <verify>
Run `npm run build` in apps/web. Verify all three layout files import and render CommandPalette + ShortcutsReference. Check no TypeScript errors with useSidebar context.
  </verify>
  <done>
Command palette and keyboard shortcuts are wired into Admin, VA, and Client layouts. Cmd+K opens command palette, ? opens shortcuts reference, and vim-style navigation shortcuts work across all dashboards.
  </done>
</task>
