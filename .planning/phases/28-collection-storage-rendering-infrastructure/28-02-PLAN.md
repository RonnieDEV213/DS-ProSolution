...
# NOTE: These server-side streaming endpoints are wired into the SellersGrid
# client-side export in Plan 05, Task 2 (step 7). For datasets exceeding 10K
# sellers, the client routes export requests to these endpoints instead of
# building CSV/JSON in the browser. For smaller datasets, client-side export
# remains the default (faster, no network round trip).
---

<objective>
Add streaming CSV and JSON export endpoints for sellers to the existing export router.

Purpose: The current SellersGrid only has client-side export. For datasets exceeding 10K sellers (projected 50-100K+), server-side streaming export prevents browser memory exhaustion. These endpoints mirror the existing records export pattern. Plan 05 wires SellersGrid to route large exports to these endpoints via a threshold check.
...
- Endpoints available for Plan 05 to wire into SellersGrid large-dataset export path.
</success_criteria>
