...
must_haves:
  truths:
    ...
    - "api.ts exports getAccessToken so downstream hooks can make authenticated requests"
  ...
  artifacts:
    ...
    - path: "apps/web/src/lib/api.ts"
      provides: "Typed seller API functions and getAccessToken export"
      contains: "sellerApi"
...
Output: Extended query-keys.ts, api.ts with sellerApi object and exported getAccessToken, pending-mutations.ts with seller case handling.
</objective>
...
<task type="auto">
  <name>Task 1: Extend query-keys.ts, add seller API functions and export getAccessToken from api.ts</name>
  <files>
    apps/web/src/lib/query-keys.ts
    apps/web/src/lib/api.ts
  </files>
...
2. In api.ts:
   - **Export `getAccessToken`:** The function is currently file-scoped (not exported). Add `export` to its declaration so downstream modules (Plan 06's sync.ts, use-collection-polling.ts) can import it for authenticated requests. Change `async function getAccessToken()` to `export async function getAccessToken()`. If it is declared as `const getAccessToken = async ()`, change to `export const getAccessToken = async ()`.
  ...
  <verify>
    ...
    - Grep confirms `export` on getAccessToken declaration in api.ts.
  </verify>
  <done>
    ...
    - getAccessToken is exported from api.ts (available for downstream Plan 06 imports).
...
- getAccessToken is exported and available for downstream imports.
...
- getAccessToken exported from api.ts for use by Plan 06 and other downstream consumers.
