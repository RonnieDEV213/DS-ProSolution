...
<objective>
Add IndexedDB persistence for collection run history and migrate useCollectionPolling to TanStack Query.

Purpose: Two remaining CONTEXT.md requirements not covered by Plans 01-05. (1) Collection run history currently fetches from server on every visit with no caching. Persisting to IndexedDB gives instant load for the history panel, matching the sellers experience. (2) useCollectionPolling currently uses raw fetch+setInterval. Migrating to TanStack Query provides automatic retry, caching, devtools visibility, and consistency with the rest of the v3 infrastructure.

Output: New collection_runs Dexie table, useSyncRunHistory hook, and refactored useCollectionPolling with TanStack Query.
</objective>
...
<task type="auto">
  <name>Task 1: Add collection_runs table to Dexie and create useSyncRunHistory hook</name>
  <files>
    apps/web/src/lib/db/schema.ts
    apps/web/src/lib/db/index.ts
    apps/web/src/lib/db/sync.ts
...
3. **Add syncCollectionRuns() to sync.ts:**
   ...
   Note: The exact API shape depends on what the backend provides. Read the `/collection/runs` endpoint response to verify. If `updated_since` isn't supported as a query param, sync all runs each time (the dataset is small -- typically <100 runs total).
...
5. **Create useSyncRunHistory hook:**
   ...
   This hook can then be consumed by collection-history.tsx and history-panel.tsx to replace their direct fetch patterns.
  </action>
  <verify>
...
export function useCollectionPolling(pollingInterval = 500) {
  ...
  const {
    ...
    queryFn: async ({ signal }): Promise<CollectionRun | null> => {
      const token = await getAccessToken(); // Use existing getAccessToken from api.ts
...
**IMPORTANT:** Import `getAccessToken` from `@/lib/api` instead of using `createClient().auth.getSession()`. This matches the pattern used in all other hooks after v3 migration. If `getAccessToken` is not exported from api.ts, it will be after Plan 01 (query-keys.ts and api.ts are modified in Plan 01, and getAccessToken is already used internally).
...
- useCollectionPolling fully migrated to TanStack Query.
