...
must_haves:
  truths:
    ...
    - "Any SSE or real-time handler that previously called setSellers() now uses db.sellers.bulkPut()"
    - "Large-dataset export (>10K sellers) can fall through to server-side streaming endpoints from Plan 02"
  artifacts:
...
6. **SSE/real-time handler migration (IMPORTANT):**
   Search the entire file and related components (progress-detail-modal.tsx, activity-feed.tsx) for any code that calls `setSellers()` in response to SSE events or real-time data pushes (e.g., new sellers arriving during an active collection run). If any such patterns exist, replace `setSellers(prev => [...prev, newSeller])` with `await db.sellers.bulkPut([newSeller])` -- useLiveQuery will reactively update the UI. Currently, the SSE handler in progress-detail-modal.tsx feeds the activity feed (not sellers-grid directly), and new sellers during runs are picked up via `refreshTrigger` (now `refetch()` from Plan 04). Verify this is correct and document any SSE->setSellers patterns found or confirm none exist.
  </action>
  <verify>
    ...
    - Grep: no `setSellers` called from any SSE/EventSource handler in collection components.
  ...
  <done>
    ...
    - Any SSE handler that previously used setSellers() now uses db.sellers.bulkPut() (or confirmed none exist).
  </done>
</task>

<task type="auto">
  <name>Task 2: Migrate add, edit, delete, export (with server-side routing), and undo operations</name>
  <files>
    apps/web/src/components/admin/collection/sellers-grid.tsx
  </files>
  <action>
...
7. **Wire export to server-side streaming for large datasets:**

   The export functions (handleExportCSV, handleExportJSON) currently build export data client-side from the sellers array. Add a threshold check: if totalCount (from useSyncSellers) exceeds 10,000 sellers, route CSV/JSON export to the server-side streaming endpoints created in Plan 02 (`/export/sellers/csv`, `/export/sellers/json`). For datasets under 10K, keep client-side export (faster, no network round trip).

   ```typescript
   const LARGE_EXPORT_THRESHOLD = 10_000;

   const handleExportCSV = useCallback(async () => {
     const exportSellers = getExportSellers(); // selected or filtered

     if (exportSellers.length > LARGE_EXPORT_THRESHOLD || totalCount > LARGE_EXPORT_THRESHOLD) {
       // Route to server-side streaming export
       const params = new URLSearchParams();
       // If only flagged are selected, pass filter
       if (exportFlaggedOnly) params.set('flagged', 'true');
       window.open(`${process.env.NEXT_PUBLIC_API_BASE_URL}/export/sellers/csv?${params}`, '_blank');
       return;
     }

     // Client-side export for small datasets (existing logic)
     // ... keep existing client-side CSV generation ...
   }, [/* deps */]);
   ```

   Apply the same pattern to handleExportJSON.

   Note: The server-side endpoints (Plan 02) use auth via the existing permission system. For the window.open approach, the auth token needs to be passed as a query parameter or the endpoint needs cookie-based auth. If the existing export endpoints for bookkeeping use a different pattern (e.g., fetch + blob download), follow that same pattern instead of window.open.

8. **Remove all remaining cleanup:**
...
10. **Verify SellersGridProps:**
  ...
  <verify>
    ...
    - Grep confirms: `LARGE_EXPORT_THRESHOLD` or server-side export routing logic present.
    - `npx tsc --noEmit` passes.
  ...
  <done>
    ...
    - Large dataset exports (>10K) route to server-side streaming endpoints (Plan 02).
    - Small dataset exports remain client-side for speed.
...
- Large exports route to server-side streaming endpoints.
- No SSE handlers reference setSellers anywhere in collection components.
</verification>
...
- Large dataset export routes to Plan 02 server-side streaming endpoints.
...
- SSE/real-time handlers verified to not use setSellers (use db.sellers.bulkPut instead).
</success_criteria>
