...
must_haves:
  truths:
    - "Record updates appear instantly in UI before server response"
    - "Failed mutations rollback to previous state"
    - "Mutations retry automatically up to 3 times on network/server errors"
    - "Retry uses exponential backoff (1s, 2s, 4s)"
    - "4xx validation errors do not retry"
    - "Local IndexedDB updated alongside TanStack Query cache"
  artifacts:
