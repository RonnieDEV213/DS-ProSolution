...
must_haves:
  truths:
    - "User can edit records while offline"
    - "Offline edits sync automatically when back online"
    - "Offline edits sync in the order they were made"
  ...
  key_links:
    ...
    - from: "apps/web/src/lib/db/pending-mutations.ts"
      ...
      via: "detectConflict called in processQueue when server responds"
...
- Queue processing logic with sequential execution and conflict detection
...
<task type="auto">
  <name>Task 2: Create pending mutations queue manager with conflict detection</name>
  <files>
    apps/web/src/lib/db/pending-mutations.ts
  </files>
  <action>
...
export async function processQueue(
  ...
  for (const mutation of pending) {
    ...
    try {
      // Execute mutation and get server response
      const serverRecord = await executeMutation(mutation);

      // Check for conflict (only for updates that return a record)
      if (serverRecord && mutation.operation === 'update') {
 ...
 * Returns the server record for update operations (needed for conflict detection).
 */
async function executeMutation(mutation: PendingMutation): Promise<BookkeepingRecord | null> {
...
IMPORTANT: The `processQueue` function accepts an `onConflict` callback. When called from SyncProvider (Plan 19-04), this callback will be `addConflict` from the sync context. This is the integration point that wires conflict detection to the UI modal.
  </action>
  <verify>
TypeScript compiles:
```bash
...
- processQueue calls detectConflict for update operations
- processQueue accepts onConflict callback to wire to SyncProvider
...
- Conflict detection wired to processQueue (calls onConflict callback)
</success_criteria>
