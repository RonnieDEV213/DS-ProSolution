...
3. Modify mutationFn to queue when offline. The key insight: onMutate already handles optimistic updates to cache and IndexedDB. When offline, mutationFn just needs to queue the operation for later sync - it does NOT need to duplicate the IndexedDB write.
...
The simplicity here: when offline, queue for later and return optimistic response. When processQueue runs after coming online, it will call api.createRecord and the server returns a real ID. The next sync will reconcile.
  </action>
  <verify>
TypeScript compiles:
```bash
...
- Create a new record
- Check IndexedDB _pending_mutations table has the create operation
- Go online
- Record syncs and gets a real ID from server
  </verify>
  <done>
