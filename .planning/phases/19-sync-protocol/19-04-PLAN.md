...
must_haves:
  truths:
    - "Conflicts detected when server version newer than pending mutation"
 ...
 * Called during sync when server returns record with updated_at > mutation timestamp.
 */
...
<task type="auto">
  <name>Task 2: Create SyncProvider for conflict queue management</name>
  <files>
    apps/web/src/components/providers/sync-provider.tsx
  </files>
  <action>
...
1. TypeScript compiles: `cd apps/web && npx tsc --noEmit`
2. App runs: `cd apps/web && npm run dev`
3. Manually trigger conflict by:
   - Open browser DevTools console
   - Get the SyncProvider's addConflict via React DevTools or expose test function
   - Add test conflict: `addConflict({ id: '1', record_id: '...', mutation_id: '...', local_values: {...}, server_values: {...}, conflicting_fields: [...], created_at: '...' })`
   - Modal should appear with field diff
