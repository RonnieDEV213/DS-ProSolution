...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/web/src/components/admin/collection/collection-history.tsx"
      to: "onRerun prop"
      via: "Re-run button in Actions column"
      pattern: "onRerun\\(.*category_ids"
---
...
- Re-run button that calls onRerun(entry.category_ids) to pre-select categories
  </action>
  <verify>File exists at apps/web/src/components/admin/collection/collection-history.tsx with Table import, handleExportRun function, and Re-run button with onRerun(entry.category_ids) call</verify>
  <done>CollectionHistory component displays history table with status, stats, per-run export, and Re-run button that invokes onRerun prop</done>
</task>
...
2. Add state for minimize and preselected categories:
```typescript
  ...
  <div className="space-y-6">
    ...
    <RunConfigPanel
      ...
      onOpenChange={(open) => {
        ...
        if (!open) setPreselectedCategories([]);  // Clear on close
      }}
      initialCategories={preselectedCategories}
      onRunStarted={handleRefresh}
    />
    {/* ... other modals ... */}
  </div>
)}
...
7. Reset preselectedCategories when modal closes to avoid stale state on next open.
  </action>
  <verify>
1. automation/page.tsx imports CollectionHistory
2. automation/page.tsx has handleCancelRun function
3. automation/page.tsx has preselectedCategories state
4. CollectionHistory onRerun sets preselectedCategories before opening modal
5. RunConfigPanel receives initialCategories={preselectedCategories}
6. preselectedCategories is cleared when modal closes
  </verify>
  <done>
...
- onRerun from history sets preselectedCategories state and opens RunConfigPanel
- RunConfigPanel receives initialCategories prop to pre-select categories from past run
...
3. Verify:
   ...
   - Click Play icon (Re-run) on a history row -> opens RunConfigPanel with categories pre-selected
...
2. Each history row has status badge, stats columns, export button, and Re-run button
3. Re-run button calls onRerun(entry.category_ids) to trigger pre-selection
4. Export dropdown has Download CSV, Download JSON, Copy to Clipboard options
5. Progress modal has minimize button that shows floating indicator
6. Cancel button in progress bar and modal calls /cancel endpoint
7. All components integrated in automation page
8. onRerun from CollectionHistory sets preselectedCategories and passes to RunConfigPanel
</verification>
...
- Admin can re-run a past collection with same categories pre-selected
</success_criteria>
