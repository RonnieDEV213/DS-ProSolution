...
must_haves:
  truths:
    ...
    - "API cost is tracked and displayed during collection run"
    - "Rate limiting is handled with retry and throttle status shown in UI"
  ...
  artifacts:
    ...
    - path: "apps/web/src/components/admin/collection/progress-bar.tsx"
      provides: "Progress bar with throttle status display"
  ...
  key_links:
    ...
    - from: "progress-bar.tsx"
      ...
      via: "progress prop"
      pattern: "rate_limited|waiting_seconds"
---

<objective>
Integrate Oxylabs scraper with collection service and update RunConfigModal to use real Amazon category selector. Update progress bar to display running API cost and throttle status.

Purpose: Connect the scraper to the collection pipeline so that starting a run actually fetches products. Update the UI to use the new category selector instead of placeholders. Display real-time cost and throttle status per CONTEXT.md requirements.

Output: Working end-to-end flow from UI category selection to Oxylabs API calls with proper progress display
</objective>
...
<task type="auto">
  <name>Task 4: Update ProgressBar to display throttle status and running cost</name>
  <files>apps/web/src/components/admin/collection/progress-bar.tsx</files>
  <action>
Update the ProgressBar component to display:
1. Running API cost with explicit label: "API cost so far: $X.XX" (per CONTEXT.md)
2. Throttle status when rate limited: "Waiting X seconds..." (per CONTEXT.md)

The backend (Task 1) stores throttle status in `checkpoint.status = "rate_limited"` with `checkpoint.waiting_seconds`.

Key changes:
1. Add `checkpoint` field to progress interface with optional `status` and `waiting_seconds`
2. Display throttle status banner when `checkpoint.status === "rate_limited"`
3. Update cost label to say "API cost so far:" instead of just "Cost:"

Update the ProgressBarProps interface to include checkpoint:
```tsx
interface ProgressBarProps {
  progress: {
    ...
    // Add checkpoint field for throttle status
    checkpoint?: {
...
Add throttle status display between the progress bar and quick info section:
```tsx
...
Update the cost display label from "Cost:" to "API cost so far:":
```tsx
{/* Cost */}
...
```
  ...
  <verify>TypeScript compiles: `cd apps/web && npx tsc --noEmit src/components/admin/collection/progress-bar.tsx 2>&1 | head -20`</verify>
  <done>ProgressBar shows "API cost so far: $X.XX" and "Waiting X seconds..." throttle status when rate limited</done>
</task>
...
5. Cost is tracked via actual_cost_cents and displayed as "API cost so far: $X.XX"
6. Throttle status displays "Waiting X seconds..." when rate limited
7. Multiple consecutive failures pause the run
...
- [ ] ProgressBar shows "API cost so far: $X.XX" during fetch
- [ ] ProgressBar shows "Waiting X seconds..." when rate limited
