...
5. **Deprecated component import check (SKEL-03 guard):** Verify that `recent-logs-sidebar` and `collection-history` are NOT actively imported anywhere:
   ```bash
   grep -rn "recent-logs-sidebar\|collection-history" --include="*.tsx" --include="*.ts" apps/web/src/app/ apps/web/src/components/ apps/web/src/hooks/ apps/web/src/lib/
   ```
   - If NO active imports are found: These deprecated files contain "Loading..." text but are dead code and do not violate SKEL-03. Document this finding in the summary.
   - If active imports ARE found: The importing file(s) must have their imports removed or the deprecated components must have their "Loading..." text replaced with skeletons before SKEL-03 can be satisfied. Flag this as a gap.
</verification>
...
- Deprecated components (recent-logs-sidebar.tsx, collection-history.tsx) verified as not actively imported, confirming their "Loading..." text does not violate SKEL-03
</success_criteria>
